...
module.exports = {
    ...
    execute(msg, args, client){
      ...
      if(!args[0]){
        return msg.channel.send(`You didn't provide any tag action, ${msg.author}!`);
      }
      const tagAction = args[0];

      if(tagAction === 'add'){
        if(validateTagName(msg, args[1]) && validateTagDescription(msg, args[2])){
...
async function taginfo(msg, tagName){
  ...
  if (tag) {
    ...
    date = d.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }) + ", " + d.toDateString();
